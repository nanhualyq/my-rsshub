...
export async function GET(request: Request) {
  try {
    ...
    const res = await fetch(
      'https://m.douban.com/rexxar/api/v2/subject_collection/movie_showing/items?start=0&count=50',
      {
        headers: {
          'User-Agent':
    ...
    if (!res.ok) {
      ...
      return new Response('Failed to fetch now-playing movies', { status: 502 })
    }

    const data: DoubanResponse = await res.json()
    let movies = data.subject_collection_items
    if (!isNaN(minRating)) {
      movies = movies.filter((item) => item.rating?.value >= minRating)
    }
    const items: RssItem[] = movies.map((item) => ({
      title: item.title,
    ...
    if (items.length === 0) {
      return new Response('No movies found', { status: 502 })
    }

    const descSuffix = !isNaN(minRating) ? `（评分 ≥ ${minRating}）` : ''
    const xml = buildRssXml(
      {
        title: `豆瓣电影 - 正在热映${descSuffix}`,
        link: 'https://movie.douban.com/',
        description: `豆瓣电影正在热映影片列表${descSuffix}`,
        language: 'zh-cn',
      },
      items,
    )
    ...
    console.error('Failed to generate now-playing RSS:', error)
