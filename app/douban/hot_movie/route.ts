...
export async function GET(request: Request) {
  try {
    ...
    const res = await fetch(
      'https://m.douban.com/rexxar/api/v2/subject_collection/movie_hot/items?start=0&count=50',
      {
        headers: {
          'User-Agent':
    ...
    if (!res.ok) {
      ...
      return new Response('Failed to fetch hot movies', { status: 502 })
    }
    ...
    const xml = buildRssXml(
      {
        title: `豆瓣电影 - 最近热门电影${descSuffix}`,
        link: 'https://movie.douban.com/',
        description: `豆瓣电影最近热门电影列表${descSuffix}`,
        language: 'zh-cn',
      },
      items,
    )
    ...
    console.error('Failed to generate hot movie RSS:', error)
